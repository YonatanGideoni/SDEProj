...
def solve_scipy(init_x, min_timestep, rtol=1e-5, atol=1e-5, method='RK45'):
    res = integrate.solve_ivp(lambda t, x: reverse_time(ode_func, t, x), (min_timestep, 1.0), init_x, rtol=rtol, atol=atol, method=method)
    return res["y"], res["t"]
...
def solve_magnani(init_x, min_timestep, steps, q=2, prior='OU', print_t=False):
    ...
    # Solve the ODE!
    ms, ts = odesolver.solve_kf(m0, P0, lambda t, x : reverse_time(ode_func, t, x), t0=min_timestep, t1=1.0, steps=steps, q=q, method=prior)
